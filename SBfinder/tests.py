...
from SBfinder.models import Course, Friend_Request, UserInfo
...
class GenerateSBList(TestCase): 
    ...
    def test_GenerateSBListViews(self): 
        ...
        response = self.client.get(reverse('SBfinder:generate_SB'))
    ...
    def test_empty_SBList(self): 
        ...
        response = self.client.get(reverse('SBfinder:generate_SB'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Your Study Buddy List")
...
class FriendRequest(TestCase):
    ...
    def setUp(self):
        ...
        self.user2 = get_user_model().objects.create_user(username='test2', password='12test12', email='test@example.com')
        self.client = Client()
        self.user2.save()
    ...
    def test_friend_request_load(self):
        ...
        self.client.login(username='test', password='12test12')
        user = auth.get_user(self.client)
        response = self.client.get(reverse('SBfinder:friend_request'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Pending Friend Requests")
