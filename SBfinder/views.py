...
def getClasses(request):
    ...
    if 'numSearch' in request.GET:# Check if the inputed search is a valid class number
        if len(request.GET['numSearch']) > 0 and len(request.GET['subjSearch']) == 0:
            messages.error(request, 'Invalid department number!')
...
def send_friend_request(request, userID):
    ...
    if created:
        messages.success(request, 'Friend request has been sent! Please wait for the other user to respond...')
...
def create_SS(request):
    ...
    messages.success(request, 'Your new study session is created! Go to the study session section to check it out!')
    # redirect the user to his/her profile where all study sessions are displayed
    return HttpResponseRedirect(reverse('SBfinder:profile', args = (current_userInfo.slug,)) + '#studySession')
...
def upload_pic(request):
    ...
    try:
        ...
        messages.error(request, "Please upload a picture of valid format (jpg, jpeg, png etc)!")
        return HttpResponseRedirect(reverse('SBfinder:setting', args=(current_user.pk,)))
